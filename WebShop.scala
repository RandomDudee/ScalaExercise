object WebShop extends App {
  ...
  case class Cart(var user: User, var order: List[OrderLine]) {
    println("user" + user.name + " bought " + order.map(_.item.name).mkString("and ") + " £" + sum)
  ...
  val purchase2 = new Cart(nozuza, List(new OrderLine(milk, 4, 1.5), new OrderLine(oranges, 4, 5)))
  val purchase3 = new Cart(komaja, List(new OrderLine(bananas, 1, 2), new OrderLine(oranges, 2, 3)))
//}
